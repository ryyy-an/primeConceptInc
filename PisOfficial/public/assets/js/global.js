...
const NOTIF_POLL_INTERVAL = 45000; // 45 seconds
let notifPollTimer = null;

window.refreshNotifications = async function () {
  try {
    const response = await fetch(
      "../include/inc.admin/admin.ctrl.php?action=get_notifications",
    );
    const result = await response.json();

    if (result && result.success) {
      updateNotifUI(result.count, result.list);
    }
  } catch (error) {
    console.error("Notif Poll Error:", error);
  }
};

function updateNotifUI(count, list) {
  ...
  if (listContainer) {
    let html = '<div class="divide-y divide-gray-50 bg-white">';
    if (list.length > 0) {
      list.forEach((n) => {
        const isUnread = n.is_read == 0;
        html += `
                    <div class="px-4 py-3 hover:bg-gray-50 transition-colors ${isUnread ? "bg-red-50/30" : ""}">
                        <div class="flex flex-col gap-1">
                            <div class="flex justify-between items-start gap-2">
                                <p class="text-xs text-gray-800 leading-relaxed font-bold">${n.title}</p>
                                ${isUnread ? '<span class="w-2 h-2 rounded-full bg-red-500 mt-1 shrink-0"></span>' : ""}
                            </div>
                            <p class="text-[11px] text-gray-500 leading-normal line-clamp-2">${n.message}</p>
                            <span class="text-[9px] text-gray-400 font-medium mt-0.5">${n.created_at}</span>
                        </div>
                    </div>
                `;
      });
      ...
      html +=
        '<div class="px-4 py-8 text-center text-xs text-gray-400 italic">No recent notifications</div>';
    }
    html += "</div>";
...
window.markNotificationsRead = async function () {
  try {
    const response = await fetch(
      "../include/inc.admin/admin.ctrl.php?action=mark_notifications_read",
    );
    const result = await response.json();
    if (result && result.success) {
      const badge = document.getElementById("notifBadge");
...
document.addEventListener("DOMContentLoaded", () => {
  refreshNotifications();
  notifPollTimer = setInterval(refreshNotifications, NOTIF_POLL_INTERVAL);
