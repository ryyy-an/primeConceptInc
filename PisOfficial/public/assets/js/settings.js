...
window.showSettingSection = function(section) {
    ...
    const activeClasses = ['bg-red-600', 'text-white', 'shadow-lg', 'shadow-red-100', 'active-setting-link'];
    const inactiveClasses = ['text-gray-400', 'hover:bg-gray-50', 'bg-transparent'];
    ...
    if (section === 'userManagement') {
        ...
        placeholderLink.classList.remove(...activeClasses);
        placeholderLink.classList.add(...inactiveClasses);
        ...
        userLink.classList.remove(...activeClasses);
        userLink.classList.add(...inactiveClasses);
    }
};
...
window.openResetModal = function(userId, fullName) {
    ...
    const passInput = document.querySelector('#resetModal input[name="new_password"]');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (confirmPasswordInput) confirmPasswordInput.addEventListener('input', checkMatch);
    ...
    if (usernameInput) {
        usernameInput.addEventListener('input', () => {
            ...
            fetch('../include/inc.admin/check_user.php', { method: 'POST', body: formData })
                .then(res => res.text())
                .then(data => {
    ...
    if (regForm) {
        regForm.addEventListener('reset', () => {
            ...
            statuses.forEach(id => {
                ...
                if (el) {
                    el.innerText = (id === 'userStatus' ? 'Required' : (id === 'nameStatus' ? 'Min 3 chars' : 'Not Matched'));
            ...
            if (stText) {
                ...
                stText.className = "text-[10px] font-medium uppercase tracking-wider text-slate-400";
            }
